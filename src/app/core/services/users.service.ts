...
export class UsersService {
  ...
  index(): Observable<User[]> {
    return this.httpClient.get<User[]>(`${environment.innoApi.baseUrl}`);
  }

  show(userId: number): Observable<User> {
    return this.httpClient.get<User>(`${environment.innoApi.baseUrl}/${userId}`);
  }

  create(user: User): Observable<User> {
    return this.httpClient.post<User>(`${environment.innoApi.baseUrl}`, user, httpOptions);
  }
